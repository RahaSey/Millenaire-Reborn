...
public class ResManager {
      ...
      if (((ResourceLocation)this.soilTypes.get(i)).equals(Mill.CROP_CACAO))
        for (Point p : this.soils.get(i)) {
          if (p.getBlock(this.building.world) == Blocks.AIR) {
            if (p.getNorth().getBlock(this.building.world) == Blocks.LOG && isBlockJungleWood(p.getNorth().getBlockActualState(this.building.world)))
              return p; 
            if (p.getEast().getBlock(this.building.world) == Blocks.LOG && isBlockJungleWood(p.getEast().getBlockActualState(this.building.world)))
              return p; 
            if (p.getSouth().getBlock(this.building.world) == Blocks.LOG && isBlockJungleWood(p.getSouth().getBlockActualState(this.building.world)))
              return p; 
            if (p.getWest().getBlock(this.building.world) == Blocks.LOG && isBlockJungleWood(p.getWest().getBlockActualState(this.building.world)))
              return p; 
          } 
        }  
    } 
  ...
  public int getNbNetherWartHarvestLocation() {
    ...
    for (int i = 0; i < this.netherwartsoils.size(); i++) {
      ...
      if (WorldUtilities.getBlock(this.building.world, p.getAbove()) == Blocks.NETHER_WART && WorldUtilities.getBlockMeta(this.building.world, p.getAbove()) >= 3)
        nb++; 
    } 
    return nb;
  }
  ...
  public int getNbSilkWormHarvestLocation() {
    ...
    for (int i = 0; i < this.silkwormblock.size(); i++) {
      ...
      if (WorldUtilities.getBlockState(this.building.world, p) == MillBlocks.SILK_WORM.getDefaultState().withProperty((IProperty)BlockSilkWorm.PROGRESS, (Comparable)BlockSilkWorm.EnumType.SILKWORMFULL))
        nb++; 
    } 
    return nb;
  }
  ...
  public Point getNetherWartsPlantingLocation() {
    ...
    for (i = start; i < this.netherwartsoils.size(); i++) {
      Point p = this.netherwartsoils.get(i);
      if (WorldUtilities.getBlock(this.building.world, p.getAbove()) == Blocks.AIR && WorldUtilities.getBlock(this.building.world, p) == Blocks.SOUL_SAND)
        return p; 
    } 
    for (i = 0; i < start; i++) {
      Point p = this.netherwartsoils.get(i);
