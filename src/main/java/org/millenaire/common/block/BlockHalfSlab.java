...
public abstract class BlockHalfSlab extends Block {
  ...
  public BlockHalfSlab(Block fullBlock) {
    super(fullBlock.getDefaultState().getMaterial(), fullBlock.getDefaultState().getMaterialColor(null, null));
  ...
  public boolean doesSideBlockRendering(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing face) {
    ...
    if (state.isOpaqueCube())
      return true; 
    BlockSlab.EnumBlockHalf side = (BlockSlab.EnumBlockHalf)state.get((IProperty)HALF);
  ...
  public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
    if (face == EnumFacing.UP && state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.TOP)
      return BlockFaceShape.SOLID; 
    return (face == EnumFacing.DOWN && state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.BOTTOM) ? BlockFaceShape.SOLID : BlockFaceShape.UNDEFINED;
  }
  
  public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
    return (state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.TOP) ? AABB_TOP_HALF : AABB_BOTTOM_HALF;
  }
  
  public int getMetaFromState(IBlockState state) {
    int i = 0;
    if (state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.TOP)
      i |= 0x8; 
    return i;
  }
  
  public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
    IBlockState iblockstate = super.onBlockPlaced(worldIn, pos, facing, hitX, hitY, hitZ, meta, placer).withProperty((IProperty)HALF, (Comparable)BlockSlab.EnumBlockHalf.BOTTOM);
  ...
  public boolean isSideSolid(IBlockState base_state, IBlockAccess world, BlockPos pos, EnumFacing side) {
    ...
    return ((state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.TOP && side == EnumFacing.UP) || (state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.BOTTOM && side == EnumFacing.DOWN));
  }
  
  public boolean isFullyOpaque(IBlockState state) {
    return (state.get((IProperty)HALF) == BlockSlab.EnumBlockHalf.TOP);
  }
